        ...
        case xPDOTransport::ACTION_INSTALL:
            ...
            foreach ($settings as $key) {
                if (isset($options[$key])) {
                    $setting = $object->xpdo->getObject('modSystemSetting', array('key' => 'uploadtousers.' . $key));
                    if ($setting != null) {
                        $setting->set('value', $options[$key]);
                        $setting->save();
                    } else {
                        $object->xpdo->log(xPDO::LOG_LEVEL_ERROR, '[Upload to Users CMP] '
                                . $key . ' setting could not be found, so the setting could not be changed.');
                    }
                }
            }
            $success = true;
